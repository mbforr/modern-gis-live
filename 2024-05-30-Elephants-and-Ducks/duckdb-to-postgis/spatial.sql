...
describe from 'https://storage.googleapis.com/citi-bikes-spatial/citibike_2023.parquet';

select count(*) as rides, start_station_name from 'https://storage.googleapis.com/citi-bikes-spatial/citibike_2023.parquet' group by 2 order by 1 desc;

create or replace table top_stations as with a as (select count(*) as rides, start_station_name from 'https://storage.googleapis.com/citi-bikes-spatial/citibike_2023.parquet' group by 2 order by 1 desc) select a.*, b.lat, b.lng from a join 'https://storage.googleapis.com/citi-bikes-spatial/citibike_stations.parquet' b on a.start_station_name = b.name;
...
create or replace table top_dest as with a as (select count(*) as rides, end_station_name from 'https://storage.googleapis.com/citi-bikes-spatial/citibike_2023.parquet' where start_station_name = 'W 21 St & 6 Ave' group by 2 order by 1 desc) select a.*, b.lng, b.lat from a join 'https://storage.googleapis.com/citi-bikes-spatial/citibike_stations.parquet' b on a.end_station_name = b.name;
